"""
Datasets to use :  - Only DCM and Control
Datasets to use :  - with Other Disorders Dropped
"""
...
SERIES_LIST = ["GSE42955", "GSE35108", "GSE3586", "GSE4172", "GSE19303" ]
DEST_DIR    = "."
OUTPUT_FILE = "DCM_combined_dataset.csv"
...
def analyze_metadata():
    ...
    for dataset_id, samples_metadata in metadata_analysis.items():
        ...
        # Count labels
        label_counts = {'DCM (1)': 0, 'Control (0)': 0, 'Unlabeled': 0}
        ...
        for sample in samples_metadata:
            ...
            if label == 1:
                label_counts['DCM (1)'] += 1
                for entry in metadata_entries:
                    if 'dcm' in entry:
                        dcm_patterns.add(entry)
            elif label == 0:
        ...
        if dcm_patterns:
            print(f"DCM-related metadata patterns ({len(dcm_patterns)}):")
            for pattern in sorted(dcm_patterns):
                print(f"  - '{pattern}'")
