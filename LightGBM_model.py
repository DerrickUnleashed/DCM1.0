...
sanitized_columns_train = [re.sub(r'[^A-Za-z0-9_]', '_', col) for col in X_train_scaled.columns]
sanitized_columns_test = [re.sub(r'[^A-Za-z0-9_]', '_', col) for col in X_test_scaled.columns]
X_train_scaled.columns = sanitized_columns_train
X_test_scaled.columns = sanitized_columns_test
...
report = classification_report(y_test, y_pred, target_names=["NORMAL", "DCM"])
...
plt.title('LightGBM ROC Curve for DCM', fontsize=14)
plt.legend(loc='lower right')
plt.grid(True, linestyle='--', alpha=0.6)
plt.savefig("DCM_model_validation.png", dpi=400)
plt.show()
